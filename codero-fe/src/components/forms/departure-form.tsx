...
export default function DepartureForm({ scheduleId }: { scheduleId: number }) {
    ...
    return (
        <form id="departure_form" className="pt-5 flex flex-col gap-2 w-fit" onSubmit={onSubmit}>
            <p className="px-3"> Masukan password untuk isi kepulangan: </p>

            <div id="departure_form_input" className="w-full relative">
                <input
            ...
            {error && <p className="flex items-center justify-center text-red">{error}</p>}

            <div id="departure_form_button" className={`pt-${error ? 0 : 4} flex justify-between`}>
                <button
                    type="reset"
                    onClick={() => {
                        setPassword("");
